    ...
    >
      <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          >
            <h3
              class="irsa text-xl font-semibold text-gray-900 dark:text-white"
            >
              تغییر دادن مشخصات
            </h3>
            <button
          ...
          <div class="p-4 md:p-5">
            <form @submit.prevent="updateData" class="space-y-4">
              ...
              <div class="mt-4">
                ...
                <input
                  style="border-radius: 16px"
                  v-model="user.phone_number_home"
                  type="text"
                  id="home_phone"
...
export default {
  ...
  methods: {
    ...
    updateData() {
      axios
        .put(
          `https://pharmedi.ir/api/shop/update-address-order/${this.productId}/`,
          this.user,
          {
            headers: {
              "Content-type": "application/json",
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            console.log("darkhast movafagh bod");
            this.$emit("updatedData");
          } else {
            console.log("darkhast movafagh nabod");
          }
        })
        .catch((error) => {
          console.error("moshkel ine:", error);
        });
    },
  },
  mounted() {
