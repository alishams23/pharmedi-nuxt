  ...
  <div class="px-4">
    <div class="my-6 row rtl">
      <div v-for="(result, index) in data" :key="result.username + 'notification'"
        ...
        <div>
          <button @click="followHandler(result, index)" href="#" v-if="(result.username != $store.state.username &&
        result.is_followed == false) ||
        result.is_followed == 'loading'
        " class="text-xs btn-material btn-animation bg-treaget btn-animation-shadow text-white">
...
export default {
  ...
  methods: {
    getData() {
      this.loading = true;
      fetch(
        `http://127.0.0.1:8000/api/followersList/?username=${this.$route.params.user}`,
        {
          headers: {
