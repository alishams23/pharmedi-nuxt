    ...
    <div v-else class="mx-2">
        ...
        <div v-for="(order, index) in data" class="bg-white px-3 my-5 rounded-2xl">
            <div class="px-4 flex justify-between flex-wrap   items-center rtl pt-4">
                <p class="   rtl text-lg pb-3 font-bold ">سبد خرید {{data.length-  index  }} </p>
                <ol class="flex items-center w-full   pb-3 max-w-sm ">
                    ...
                    :class="order.status == 'delivered' ? 'text-blue-600  '  : ''"
                    >
                        <span
                        :class="order.status == 'delivered' ? 'bg-blue-100'  : 'text-gray-500'"
            ...
            <section aria-labelledby="cart-heading" class=" pb-3">
                ...
                <ul role="list" class=" py-2  rtl">
                    <li v-for="item in order.items" :key="item.id" class="flex py-3 bg-gray-50 border my-3 rounded-xl px-4 sm:py-10 items-center">
              ...
              <div class="flex items-center flex-wrap rtl">
                  <div class="rtl font-bold text-xs my-2 mx-3">کد پیگیری : <span class="text-gray-500 font-light">{{ order.tracking_code }}</span></div>
                  <div class="rtl font-bold text-xs my-2 mx-3 ">کد رهگیری اداره ی پست  : <span class="text-gray-500 font-light">{{ order.post_code == null ? 'صادر نشده' : order.post_code }}</span></div>
                  <div class="rtl font-bold text-xs my-2 mx-3 "> تاریخ ایجاد  : <span class="text-gray-500 font-light">{{ order.jalali_time }}</span></div>
              </div>
            </section>
...
<script>
import { IconPackage, IconPackageExport, IconCheckbox } from '@tabler/icons-vue'
